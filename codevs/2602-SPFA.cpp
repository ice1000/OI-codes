...
double distance (int a, int b) { return sqrt(squared(x[a] - x[b]) + squared(y[a] - y[b])); }
...
int main (int argc, char *argv[]) {
  ...
  for (i = 0; i < m; ++i) {
    scanf("%i %i", &j, &k);
    es[i] = *new Edge(k, distance(j, k), head[j]);
    head[j] = i;
  }
  /* SPFA */
  scanf("%i %i", &fr, &en);
  ...
  while (!q->empty()) {
  	...
  	for (j = head[i]; j != -1; j = next[j]) {
  		if (d[i] + es[j].w < d[es[j].to]) {
  			d[es[j].to] = d[i] + es[j].w;
  			if (!inq[es[j].to]) {
  				++inq[es[j].to];
  				q->push(es[j].to);
				}
