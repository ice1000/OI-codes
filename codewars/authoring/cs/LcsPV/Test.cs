...
public class Test
{
		...
		private static bool IsSubsequenceOf(string sub, string full)
		{
				return "" == sub
						|| "" != full && (sub[0] == full[0]
		...
		{
				var res = LcsClass.Lcs(a, b);
				Assert.IsTrue(IsSubsequenceOf(res, a));
				Assert.IsTrue(IsSubsequenceOf(res, b));
				Assert.AreEqual(Lcs(a, b).Length, res.Length);
		}
