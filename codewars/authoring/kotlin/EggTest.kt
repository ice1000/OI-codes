import java.math.BigInteger
import java.util.Random

import java.math.BigInteger.ZERO
import java.math.BigInteger.valueOf
import org.junit.Assert.*

class FabergeTest {
	fun sol(n: BigInteger, m: BigInteger): BigInteger {
		var n = n
		if (n == ZERO || m == ZERO) return ZERO
		if (n > m) n = m
		var c = valueOf(1)
		var b = valueOf(1)
		var a = valueOf(0)
		while (c <= n) {
			val d = (m + valueOf(1) - c) * b / c
			c += valueOf(1)
			b = d
			a = a.add(d)
		}
		return a
	}

	private fun test(a: Int, b: Int, shouldBe: Int) {
		assertEquals(valueOf(shouldBe.toLong()), height(valueOf(a.toLong()), valueOf(b.toLong())))
	}

	private fun test(a: String, b: String, shouldBe: String) {
		assertEquals(BigInteger(shouldBe), height(BigInteger(a), BigInteger(b)))
	}

	private fun test(a: String, b: String) {
		assertEquals(sol(BigInteger(a), BigInteger(b)), height(BigInteger(a), BigInteger(b)))
	}

	@org.junit.Test
	@Throws(Exception::class)
	fun basicTests() {
		test(1, 51, 51)
		test(2, 1, 1)
		test(4, 17, 3213)
		test(16, 19, 524096)
		test(23, 19, 524287)
	}

	@org.junit.Test
	@Throws(Exception::class)
	fun advancedTests() {
		test("13", "550", "60113767426276772744951355")
		test("271", "550", "1410385042520538326622498273346382708200418583791594039531058458108130216985983794998105636900856496701928202738750818606797013840207721579523618137220278767326000095")
		test("531", "550", "3685510180489786476798393145496356338786055879312930105836138965083617346086082863365358130056307390177215209990980317284932211552658342317904346433026688858140133147")
	}

	@org.junit.Test
	fun seriousTests() {
		test("311", "10131")
		test("6511", "11731")
		test("11111", "12911")
		test("99999", "13911")
	}

	@org.junit.Test
	fun randomTests() {
		val random = Random(System.currentTimeMillis())
		test(random.nextInt(20000).toString() + "", random.nextInt(20000).toString() + "")
	}
}
