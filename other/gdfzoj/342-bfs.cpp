...
#define add(x, y) q.push(*new node(x, y, __min(no.min, a[x][y]), __max(no.max, a[x][y]))), ++visit[x][y];
...
typedef struct node {
	...
	node(int _x = 0, int _y = 0, int _max = a[0][0], int _min = a[0][0]) :
...
int main(int argc, const char *argv[]) {
	freopen("C:\\Users\\ice1000\\Desktop\\GDFZOJ\\data\\road\\road8.in", "r",
	        stdin);
